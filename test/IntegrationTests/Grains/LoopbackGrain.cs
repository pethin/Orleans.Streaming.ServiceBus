...
namespace IntegrationTests.Grains
{
    ...
    {
        Task<TestResult<TestEvent>?> SendTestEvent(TimeSpan delay);
    }
    ...
    public class LoopbackGrain : BaseTestGrain, ILoopbackGrain
    {
        ...
        {
            var provider = GetStreamProvider(Constants.ServiceBusStreamProvider);
        ...
        public async Task<TestResult<TestEvent>?> SendTestEvent(TimeSpan delay)
        {
            var streamProvider = GetStreamProvider(Constants.ServiceBusStreamProvider);
            var stream = streamProvider.GetStream<TestEvent>(this.GetPrimaryKey(), nameof(LoopbackGrain));
            await stream.OnNextAsync(_model!);

            await Task.WhenAny(_completion!.Task, Task.Delay(delay));
